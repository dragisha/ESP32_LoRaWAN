...
Maintainer: Miguel Luis ( Semtech ), Gregory Cristian ( Semtech ) and Daniel J
*/
#include <math.h>
#include "board.h"
 ...
 * LoRaMac bands
 */
RTC_DATA_ATTR static Band_t Bands[LORA_MAX_NB_BANDS] =
{
    BAND0,
};
 ...
 * LoRaMAC channels
 */
RTC_DATA_ATTR static ChannelParams_t Channels[LORA_MAX_NB_CHANNELS] =
{
    LC1,
    LC2,
    LC3,
...
static void CalculateBackOff( uint8_t channel )
{
    ...
    AggregatedTimeOff = AggregatedTimeOff + ( TxTimeOnAir * (uint64_t)AggregatedDCycle - TxTimeOnAir );
}
...
LoRaMacStatus_t LoRaMacInitialization( LoRaMacPrimitives_t *primitives, LoRaMacCallback_t *callbacks, enum eDeviceClass deviceClass)
{
    ...
    RepeaterSupport = false; // indicates if the node supports repeaters;
    ...
    LoRaMacParamsDefaults.ChannelsMask[0] = 0x00FF;//bit 15~0
...
//    	Channels[i].Frequency = 902.3e6 + ( i - 8 ) * 200e3;
